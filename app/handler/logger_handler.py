...
class TimedRotatingFileHandler(BaseRotatingHandler):
    ...
    def clean_log_zip(self):
        ...
        """
        # 获得3个月以前的时间
        clean_date = datetime.datetime.strptime(datetime.datetime.now().strftime('%Y-%m-%d'),
    ...
    def doRollover(self):
        ...
        if self.backup_count > 0:
            ...
            if len(files_to_backup) >= self.backup_count:
                ...
                tar_file_name = (filename + '.' + files_to_backup[0].split('.')[-1] + '_' +
                                 files_to_backup[self.backup_count - 1].split('.')[-1] + '.tar.gz')
